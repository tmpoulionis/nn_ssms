...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    // Get photonic activation types
    const PhotonicActivationType delta_act_type = params.delta_activation_type;
    const PhotonicActivationType gate_act_type = params.gate_activation_type;
    const bool use_delta_activation = kDeltaSoftplus || 
        (delta_act_type != PhotonicActivationType::STANDARD);
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int i = 0; i < kNItems; ++i) {
            ...
            float delta_raw = float(delta_vals_load[i]) + delta_bias;

            // MODIFIED: Use photonic activation for delta
            if (use_delta_activation) {
                delta_vals[i] = photonic_delta_activation_fwd(
                    delta_raw, 
                    delta_act_type,
                    kDeltaSoftplus
                );
            } else {
                delta_vals[i] = delta_raw;
            }
        }
        ...
        // MODIFIED: Apply photonic delta activation backward
        if (use_delta_activation) {
            __syncthreads();
            input_t delta_vals_reload[kNItems];
            load_input<Ktraits>(delta, delta_vals_reload, smem_load, params.seqlen - chunk * kChunkSize);
            delta -= kChunkSize;
            #pragma unroll
            for (int i = 0; i < kNItems; ++i) {
                float delta_raw = float(delta_vals_reload[i]) + delta_bias;
                ddelta_vals[i] = photonic_delta_activation_bwd(
                    delta_raw,
                    ddelta_vals[i],
                    delta_act_type,
                    kDeltaSoftplus
